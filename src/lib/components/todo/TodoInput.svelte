<script>
	import { deleteTodo, editTodo, toggleComplete, todos } from './../../store/TodoStore.js';
	export let todo;
...
<div class="flex items-center justify-between rounded-md border-2 border-gray-one px-5 py-4">
	<div class="flex w-full max-w-lg items-center justify-start">
		<label for={`${todo.id}-chekcbox`} class="sr-only">Complete Todo</label>
		<input
			id={`${todo.id}-checkbox`}
			type="checkbox"
			on:change={() => toggleComplete(todo.id)}
			checked={todo.complete}
			class="h-4 w-4 rounded border border-gray-three bg-cream-four text-green-four focus:border-green-five focus:outline focus:outline-2 focus:outline-offset-2"
		/>
		...
		<input
			...
			value={todo.text}
			class="text-ellpsis ml-5 flex-1 rounded-none border-x-0 border-t-0 border-b border-dashed border-b-gray-two bg-cream-four px-0 pb-1 text-base font-normal text-gray-three placeholder:text-gray-two focus:border-gray-three focus:outline-none focus:ring-0"
		/>
		<span
			class="{completeClass} text-gray-5 ml-5 hidden rounded-full py-0.5 px-2 text-sm font-normal md:block"
		>
	...
	<button
		...
		class="lm-4 rouded-md group flex items-center justify-center bg-cream-four p-2 hover:bg-steel-one focus:outline-none focus-visible:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-green-five"
	>
